...
def _metax_get_default_config(
    ...
    if (
        gemm_stage == "gemm1"
        and M <= 8
        and swap_ab_shape == (512, 2048, 4096, 10)
    ):
        config["SWAP_AB"] = True
    ...
    # MC550-validated range profile for Qwen3.6 I=512.  GEMM1 and GEMM2
    # intentionally use the same BLOCK_SIZE_M so routed-token alignment and
    # correctness are preserved.  The rule does not match p32768 token
    # counts (which jump from 2048 to 16384).
    if 4096 <= M < 8192:
...
def _should_defer_router_weight_to_sum(
    args, kwargs, positional_index: int
) -> bool:
...
def _metax_moe_config_patch(
    ...
    with _PATCH_LOCK:
        ...
        if defer_router_weight_to_sum:
            ...
            generic_fused_moe.dispatch_fused_moe_kernel = (
                dispatch_with_deferred_weight
            )
